...
DELIMITER //
CREATE PROCEDURE get_department_employees(IN dept_name VARCHAR(50))
BEGIN
...
DELIMITER //
CREATE PROCEDURE get_max_salary(IN dept_name VARCHAR(50), OUT max_sal DECIMAL(10,2))
BEGIN
...
DELIMITER ;
-- Call the procedure
CALL get_max_salary('IT', @max_salary);
SELECT @max_salary;



DELIMITER //
CREATE PROCEDURE salarymax(IN dept_name VARCHAR(10), OUT salaryss DECIMAL(10,2))
BEGIN
    SELECT MAX(salary) INTO salaryss
    FROM employees
    WHERE department = dept_name;
END //
DELIMITER ;
-- Call the procedure
CALL salarymax('IT', @salaryss);
SELECT @salaryss;
